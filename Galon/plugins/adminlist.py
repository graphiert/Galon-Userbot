...
from .. import HANDLER
from ..utils.parser import mention_html, mention_markdown

from .help import HELP_CMD


@Client.on_message(filters.me & filters.command(["admins", "adminlist"], HANDLER))
async def adminlist(client: Client, message):
    replyid = None
    toolong = False
...
@Client.on_message(
    filters.me & filters.command(["reportadmin", "reportadmins", "report"], HANDLER)
)
async def report_admin(client: Client, message):
...
@Client.on_message(filters.me &
                  filters.command(["everyone", "tagall"], HANDLER))
async def tag_all_users(client: Client, message):
    await message.delete()
    if len(message.text.split()) >= 2:
        text = message.text.split(None, 1)[1]
    else:
...
@Client.on_message(filters.me & filters.command(["botlist", "bots"], HANDLER))
async def get_list_bots(client: Client, message):
    replyid = None
    if len(message.text.split()) >= 2:
        chat = message.text.split(None, 1)[1]
        grup = await client.get_chat(chat)
